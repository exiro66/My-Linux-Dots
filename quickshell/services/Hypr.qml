...
QtObject {
    ...
    // for the per-monitor workspace strip. [{ name, workspace }]
    ...
    // Sorted global ids of the workspaces currently living on a monitor.
    // Positions in this list are what `r~N` addresses: slot N of monitor M.
    function workspacesOn(name) {
        var out = [];
        try {
            for (var i = 0; i < workspaces.length; i++) {
                var w = workspaces[i];
                if (w && w.monitor && w.monitor.name === name)
                    out.push(w.id);
            }
        } catch (e) {}
    ...
    // Live active workspace id of a monitor, 0 if unknown.
    function activeOn(name) {
        try {
            var mons = Hyprland.monitors ? Hyprland.monitors.values : [];
            for (var i = 0; i < mons.length; i++)
                if (mons[i].name === name && mons[i].activeWorkspace)
                    return mons[i].activeWorkspace.id;
        } catch (e) {}
        return 0;
    }

    // "HDMI-A-1" → "HDMI". Short enough for the notch lip.
    function shortMonitor(name) {
        var s = "" + name;
        var dash = s.indexOf("-");
