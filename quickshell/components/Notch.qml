...
Item {
    ...
    // Screen this window lives on, set by NotchWindow. Only this window may
    // morph into the Command Center; the rest stay a peek.
    property string screenName: ""
    // True while this window's monitor has the focus. The shell paints on
    ...
    anchors.fill: parent
    // One monitor at a time: the sheet's window while the center is open
    // (or opening), the focused monitor otherwise. The other screen shows
    // nothing — no peek, no empty panel.
    visible: UiState.peekVisible && (notch.centerHere || (!notch.centerActive && notch.screenFocused))
    ...
    Item {
        ...
        readonly property bool live: UiState.peekVisible && notch.visible
        readonly property bool sheetHere: UiState.centerVisible && notch.centerHere
        // Just below the body while peeking (a catchable lip), the body's
        // own bottom edge once the sheet is open (past it is click-away).
        readonly property real grabBottom: visual.y + body.y + body.height + (sheetHere ? 0 : 22)
        ...
        height: !live ? 0 : sheetHere ? commandCenter.grabZone : grabBottom
        z: 10
