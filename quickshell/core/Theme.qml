...
QtObject {
    ...
    // ── typography ───────────────────────────────────────────────
    // `look.font` / `look.mono_font`, used verbatim as the family name.
    // Deliberately NO appended fallback list: QtQuick resolves a
    // comma-separated family differently than a single one (verified by
    // pixel render test), so the pick must stand alone. fontconfig still
    // covers missing glyphs underneath.
    readonly property string fontSans: Settings.font
    readonly property string fontMono: Settings.monoFont
