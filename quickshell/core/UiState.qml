...
QtObject {
    ...
    //
    // alwaysPeek: park the notch on screen at all times instead of sliding
    // it out on hover. Every path that would retire the shape to hidden
    // (toHidden, _leave, the orphan guard) stops at peek instead, and the
    ...
    // The notch overlay exists once per monitor but UiState is global, so
    // an expansion would morph on every screen at once. centerScreen pins
    // the sheet to the monitor it was summoned from; the other windows keep
    // rendering the peek. Set on every programmatic open (keybind, via
    // openOn) and on tap / drag (the pointer's own window).
    property string centerScreen: ""
    function noteScreen(screenName) {
        centerScreen = screenName || "";
    }
    ...
    onPeekOrphanedChanged: if (peekOrphaned && !alwaysPeek)
    ...
    onPeekVisibleChanged: {
        ...
        // Fully retired: release the pinned monitor so a stale centerScreen
        // can't keep authorizing peeks on a screen nobody is looking at.
        if (!alwaysPeek)
            centerScreen = "";
        _grant();
    }
